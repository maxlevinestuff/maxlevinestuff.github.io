...
function doFade() {
	...
	if (!fadeBackNowOneMoreTime) {
		...
		fade += 0.0007;
		if (fade >= 1.1) {
			window.location.replace('../4fall/fall.html');
		}
	}
}
...
function updateView(timestamp) {
	...
	if (state != 4) {
		...
		if (state == 0 || state == 1 || state == 3) {
			...
			ctx.save();
			ctx.rotate(rotation*(Math.PI/180));
			ctx.fillText("You have limited neurons", canvas.width/2, 130);
			ctx.restore();
		}
		if (state == 2) {
			ctx.font = '60px "handwritten"';
		...
		if (state == 3) {
			...
			if (isClosedLoop()) {
				...
				ctx.save();
				ctx.rotate(rotation*(Math.PI/180));
				ctx.fillText("Too complex", canvas.width/2, canvas.height/2 - 32.5);
				ctx.restore();

				var rotation = Math.sin(time + 3) - 0.5;
				ctx.save();
				ctx.rotate(rotation*(Math.PI/180));
				ctx.fillText("You cannot exceed 5 neurons", canvas.width/2, canvas.height/2 + 32.5);
				ctx.restore();
			} else {
				var rotation = Math.sin(time + 2) - 0.5;
				ctx.save();
